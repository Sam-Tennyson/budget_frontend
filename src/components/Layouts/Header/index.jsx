...
const Header = () => {
  ...
  const { openModal: isWriteModal, handleCloseModal: closeWriteModal, handleOpenModal: openWriteModal } = useModal();
  ...
  return (
    <>
      <ReactModal isOpen={isWriteModal} handleToggle={closeWriteModal} title={'Budget' || CONSTANTS.LABELS.ADD_BUDGET}>
        <CustomModalBody>
                        ...
                        values.budget_list.map((budget_list, index) => (
                          <>
                            <div className="row item-array  position-relative p-0  rounded" key={index}>
                              ...
                              {index != '0' ? <i className="bx bx-x" onClick={() => remove(index)}></i> : null}
                            </div>
                            <div className="field-required-error">
                              {errors?.budget_list?.[index]?.budget_name || errors?.budget_list?.[index]?.budget_cost}
                            </div>
                          </>
                        ))}

                      <i
                        className="bx bx-plus border rounded p-2"
                        onClick={() => push({ budget_name: '', budget_cost: '' })}></i>
                    </>
                  )}
                />

                <button type="submit" className="budget-button col-md-6 mx-auto mb-2" disabled={isLoading}>
                  {isLoading ? <ShowLoader /> : CONSTANTS.LABELS.ADD}
                </button>
              </Form>
            )}
          </Formik>
      ...
      <header className="px-4 header d-flex justify-content-between align-items-center">
        <em onClick={() => navigate({ pathname: ROUTE_CONSTANTS.HOME })}>
          <img src={IMAGES.budgetIcon} alt="icon" />
        </em>
        <h3 className="heading_title"> {CONSTANTS.LABELS.BUDGET_CONTROLLER} </h3>
        {isOpen ? (
          <i className="bx bx-x" id="hamburger-icon" onClick={handleToggle}></i>
        ) : (
          <i className="bx bx-menu" id="hamburger-icon" onClick={handleToggle}></i>
        )}
        <nav className={`navbar p-0 my-nav ${!isOpen ? 'd-none' : ''} `}>
          {TAB_DATA.map(item => (
            <a key={item?.id} onClick={() => handleTab(item)}>
