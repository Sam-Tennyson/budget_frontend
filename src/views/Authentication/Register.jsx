            ...
            {({ values, errors }) => (
              <Form>
                <div className="field-class position-relative col-md-12 mx-auto mb-3">
                  ...
                  <TextField name={'name'} placeholder={CONSTANTS.PLACEHOLDER.NAME} />
                </div>

                <div className="field-class position-relative col-md-12 mx-auto mb-3">
                  <i class="bx bxl-gmail"></i>
                  <TextField name={'email'} placeholder={CONSTANTS.PLACEHOLDER.EMAIL} />
                </div>

                <div className="field-class position-relative col-md-12 mx-auto mb-3">
                  <i className="bx bx-mobile"></i>
                  <TextField name={'phone'} placeholder={CONSTANTS.PLACEHOLDER.PHONE} maxLength={10} />
                </div>
                ...
                <div className="field-class position-relative col-md-12 mx-auto mb-3">
                  ...
                  <span className="password-class">
                    {!showPassword ? (
                      <img src={IMAGES.eyeOff} alt="eye-off" onClick={() => setShowPassword(prev => !prev)} />
                    ) : (
                      <img src={IMAGES.eyeOn} alt="eye-on" onClick={() => setShowPassword(prev => !prev)} />
                    )}
                  </span>
                </div>

                <p
                  className="my-cursor-pointer text-end set-primary-color text-bold"
                  onClick={() => navigate(ROUTE_CONSTANTS.LOGIN)}>
                  If registered, Please login!
                </p>

                <button type="submit" className="budget-button col-md-6 mx-auto mb-2" disabled={isLoading}>
                  {isLoading ? <ShowLoader /> : CONSTANTS.LABELS.REGISTER}
                </button>
              </Form>
            )}
          </Formik>
