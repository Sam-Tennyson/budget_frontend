...
const BudgetHistory = () => {
  ...
  useEffect(() => {
    if (selectMonth) {
      setBudgetData(() => [
        ...budgetListRed.filter(item => moment(item?.budget_date, 'DD-MM_YYYY').format('MMMM') == selectMonth?.value),
      ]);
      return;
    }
  ...
  return (
    <>
      {/* Edit Budget Modal  */}
      <ReactModal isOpen={openEdit} handleToggle={closeEditModal} title={CONSTANTS.LABELS.BUDGET}>
        <CustomModalBody>
          <Formik
            initialValues={{
            ...
            {({ values, setFieldValue, errors }) => (
              <Form>
                ...
                <button type="submit" className="budget-button col-md-6 mx-auto mb-2" disabled={isLoadingEdit}>
                  {isLoadingEdit ? <ShowLoader /> : CONSTANTS.LABELS.EDIT}
                </button>
              </Form>
            )}
          </Formik>
      ...
      <div className="d-flex justify-content-end">
        <ReactSelect value={selectMonth} onChange={e => setSelectedMonth(e)} options={monthFilterOptions} />
        {selectMonth ? (
          <button className="btn btn-danger ms-2" onClick={() => setSelectedMonth('')}>
            Reset
          </button>
        ) : null}
      </div>
